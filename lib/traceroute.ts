...
const RAMU_TRACE: TraceNode[] = [
  { id: "portal", office: "Member Portal", designation: "Automated eligibility engine", statutoryDeadlineDays: 1, daysHeld: 12, rule: "Claim intake and automated validation", breached: true },
  { id: "field", office: "Field Office", designation: "Accounts Officer", statutoryDeadlineDays: 7, daysHeld: 18, rule: "Claim verification queue", breached: true },
  { id: "regional", office: "Regional Office", designation: "Regional PF Commissioner", statutoryDeadlineDays: 15, daysHeld: 60, rule: "Disposition and grievance supervision", breached: true },
  { id: "cpc", office: "Central Processing Centre", designation: "Zonal Additional CPFC", statutoryDeadlineDays: 30, daysHeld: 0, rule: "Escalation destination", breached: false }
];

function tatNodes(debitedAt: string, now: number): TraceNode[] {
  const elapsed = calendarDaysSince(debitedAt, now);
  return [
...
export function traceSummary(caseId: string, now: number = Date.now(), facts?: any) {
  const isTat = caseId.includes("irctc");
  const debitedAt = facts?.debitedAt || SYNTHETIC_TXN.debitedAt;
  
  let nodes = EPFO_TRACE;
  if (isTat) {
    nodes = tatNodes(debitedAt, now);
  } else if (caseId.includes("ramu")) {
    nodes = RAMU_TRACE;
  }
  
  const blocker = [...nodes].reverse().find((node) => node.breached) ?? nodes[0];
  const daysOverdue = isTat
    ? Math.max(0, calendarDaysSince(debitedAt, now) - 5)
...
export function escalationLetter(caseId: string, facts?: any): string {
  const { blocker, daysOverdue, tatCompensationAccrued } = traceSummary(caseId, Date.now(), facts);
  const isTat = caseId.includes("irctc");
