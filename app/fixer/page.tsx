...
type CaseData = { id: string; title: string; status: string; facts: { nameAsPerEmployer: string; nameAsPerAadhaar: string; bankIfsc: string; bankIfscValid: boolean }; events: { id: string; actor: string; type: string; ts: number; hash: string; payload: Record<string, unknown> }[] };
...
export default function Fixer() {
  ...
  const [lastAction, setLastAction] = useState<{ action: string; summary: string; detail: string } | null>(null);
  const [busy, setBusy] = useState(false);

  async function load() {
    const [caseResponse, proofResponse, traceResponse] = await Promise.all([fetch(`/api/case/${CASE_ID}`), fetch("/api/prove/pension"), fetch("/api/traceroute")]);
  ...
  useEffect(() => { void load(); }, []);
  async function runStep() { setBusy(true); const response = await fetch("/api/agent/step", { method: "POST" }); const payload = await response.json(); setLastAction(payload.result); setCaseData(payload.case); setVerified(payload.verification?.valid === true); setBusy(false); }
  async function restart() { setBusy(true); await fetch(`/api/case/${CASE_ID}`, { method: "POST" }); setLastAction(null); await load(); setBusy(false); }
    ...
    <div className="mx-auto max-w-7xl">
      ...
      {!caseData ? <p className="mt-10 text-slate-400">Loading case ledger…</p> : <div className="mt-7 grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <section className="space-y-6">
          <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs text-slate-500">CASE {caseData.id}</p><h2 className="text-xl font-semibold">{caseData.title}</h2></div><span className={`rounded-full px-3 py-1 text-xs font-bold ${caseData.status === "RESOLVED" ? "bg-emerald-500/15 text-emerald-300" : "bg-amber-500/15 text-amber-300"}`}>{caseData.status}</span></div>
            <div className="mt-5 grid gap-3 sm:grid-cols-2"><div className="rounded bg-slate-950 p-3 text-sm"><p className="text-xs text-slate-500">REQUESTED MEMBER ID</p><b>{caseData.facts.nameAsPerEmployer}</b></div><div className="rounded bg-slate-950 p-3 text-sm"><p className="text-xs text-slate-500">PRIMARY UAN</p><b>{caseData.facts.nameAsPerAadhaar}</b><p className="mt-1 text-xs text-emerald-400">Exact match — rejection contradicted.</p></div></div>
            ...
            {lastAction && <div className="mt-4 rounded border border-cyan-500/30 bg-cyan-500/10 p-3"><p className="text-xs font-bold text-cyan-300">{eventLabel(lastAction.action)}</p><p className="mt-1 font-semibold">{lastAction.summary}</p><p className="mt-1 text-sm text-slate-300">{lastAction.detail}</p></div>}
          </div>
          ...
          {proof && <div className="rounded-xl border border-fuchsia-500/30 bg-fuchsia-500/5 p-5"><p className="text-xs font-bold tracking-widest text-fuchsia-300 uppercase">RuleGuard / mechanically proven</p><h2 className="mt-1 text-xl font-semibold">No valid outcome exists for {proof.domain}</h2><ol className="mt-3 list-decimal space-y-1 pl-5 text-sm text-slate-300">{proof.proofSteps.map((step) => <li key={step}>{step}</li>)}</ol><p className="mt-4 text-sm text-fuchsia-200">Route around: {proof.suggestedRouteAround}</p></div>}
        </section>
        <aside className="space-y-6">
          {trace && <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-5"><p className="text-xs font-bold tracking-widest text-amber-300 uppercase">Kaun Zimmedar? / File traceroute</p><h2 className="mt-1 text-xl font-semibold">The blocking node is visible.</h2><div className="mt-5 space-y-2">{trace.nodes.map((node, index) => <div key={node.id}><div className={`rounded border p-3 ${node.breached ? "border-red-500/50 bg-red-500/10" : "border-slate-700 bg-slate-950"}`}><div className="flex justify-between gap-2"><b className="text-sm">{node.office}</b><span className={node.breached ? "text-xs text-red-300" : "text-xs text-slate-400"}>{node.breached ? "BREACHED" : "ESCALATION TARGET"}</span></div><p className="mt-1 text-xs text-slate-300">{node.designation} · held {node.daysHeld}d / deadline {node.statutoryDeadlineDays}d</p></div>{index < trace.nodes.length - 1 && <div className="ml-5 h-4 border-l border-slate-600"/>}</div>)}</div><div className="mt-5 rounded bg-amber-500/10 p-3"><p className="text-xs text-amber-200">SLA CLOCK</p><p className="text-2xl font-bold text-amber-300">₹{trace.tatCompensationAccrued.toLocaleString("en-IN")}</p><p className="text-xs text-slate-400">{trace.daysOverdue} overdue days × ₹100/day · synthetic demo calculator</p></div><details className="mt-4"><summary className="cursor-pointer text-sm text-cyan-300">View pre-addressed escalation draft</summary><pre className="mt-3 whitespace-pre-wrap rounded bg-slate-950 p-3 text-xs text-slate-300">{trace.escalationLetter}</pre></details></div>}
          <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-5"><h2 className="font-semibold">What is real in this prototype?</h2><ul className="mt-3 space-y-2 text-sm text-slate-400"><li>✓ SHA-256 hash-chain verification</li><li>✓ Deterministic, allow-listed agent actions</li><li>✓ Interval-logic deadlock proof</li><li>✓ Evidence-backed traceroute and escalation draft</li><li className="text-slate-500">◌ Portal, case facts, deadlines, and outcomes are synthetic.</li></ul></div>
        </aside>
      </div>}
    </div>
  </main>;
