...
**FIXER.OS is built as an accountability layer.** Instead of helping the citizen submit to a broken portal, it records the portal's behavior, cross-examines it against verified citizen facts, calculates statutory delays and penalty fees, and generates court-ready escalation files.

```mermaid
graph TD
    subgraph Facts
        A[Verified Citizen Facts]
    end
    subgraph Portal
        B[Mock EPFO Portal]
    end
...
*   **Path:** [`lib/ledger.ts`](lib/ledger.ts)
...
*   **Path:** [`lib/prover.ts`](lib/prover.ts)
...
*   **Path:** [`lib/traceroute.ts`](lib/traceroute.ts)
...
*   **Path:** [`lib/preflight.ts`](lib/preflight.ts)
...
*   **Path:** [`lib/agent.ts`](lib/agent.ts) and [`lib/llm.ts`](lib/llm.ts)
...
*   **Path:** [`lib/i18n.tsx`](lib/i18n.tsx)
...
*   **Rationale:** Fast load times (~103 kB first load) and responsiveness are critical for citizens on low-bandwidth mobile networks. Fully keyboard-operable controls (focus-visible rings, semantic buttons) and 100% English-to-Hindi bilingual parity — guarded by a parity test — ensure the interface is accessible to citizens across diverse backgrounds.
...
| **1:20 - 1:35** | **SLA Clock & Traceroute.** Point to the **SLA CLOCK** showing `₹2,600` accrued and the red node on the **Regional Office** traceroute. Click *"Download Letter"*. | "Step four runs the compensation clock: twenty-six hundred rupees — and the same ₹100-a-day rule drives our IRCTC case, which mirrors RBI's TAT law exactly. Step five blames the Regional Commissioner, twenty-six days overdue, and downloads this pre-addressed escalation draft." |
...
Ledger, proofs, playbook engine, in-flow provenance verifier, and bilingual English/Hindi interface are real today; the portal and facts are mocks. The same engine already runs a second vertical — RBI TAT payment recovery (IRCTC). Safe production path: Postgres adapter and DigiLocker-authenticated ingestion. Ownership, not another form.
